...
class _ForgotPasswordScreenState extends State<ForgotPasswordScreen> {
  ...
  Future<void> _sendOtp() async {
    ...
    if (precheck['success'] != true) {
      ...
      _showSnack(
        precheck['message'] ?? 'Unable to start password reset.',
        error: true,
      );
      return;
    }
    ...
    if (backupRes['success'] == true) {
      ...
      _startTimer();
      _showSnack('OTP sent to +91$phone via backup service');
      return;
    }
  ...
  Future<void> _resetPassword() async {
    ...
    if (_usingServerOtpFallback) {
      res = await AuthService.resetPassword(phone, otp, newPass);
    } else {
      ...
      res = await AuthService.resetPasswordWithFirebase(
        _firebaseIdToken!,
        phone,
        newPass,
      );
